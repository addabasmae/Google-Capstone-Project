...
```
  ...
  2. Import csv files into Rstudio by using read_csv and change their names to make them more understandable. These files contain data regarding the monthly trips from 12-2023 to 11-2024. 
```{r}
...
```
  ...
  6. Based on our data examination, we identified a significant number of missing values in station data columns. Before proceeding with data cleaning, we need to examine patterns by rideable_type (classic bike, electric bike, electric scooter) and member_casual (casual, member):
```{r}
